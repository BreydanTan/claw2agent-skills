...
description: Generate music, melodies, and audio compositions via AI music APIs.
user-invocable: true
metadata: {"openclaw.category":"iot-media","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"music, audio, generation, ai","openclaw.requires":{"env":["MUSIC_GENERATOR_API_KEY"]}}
---
...
- "generate track" related requests
- "get track" related requests
- "list tracks" related requests
- User explicitly mentions "Music" or the related platform/service
...
```json
{
  "action": "<one of: generate_track | get_track | list_tracks | get_genres>",
...
const result = await execute(
  { action: "generate_track" },  // missing required params
  context
);
