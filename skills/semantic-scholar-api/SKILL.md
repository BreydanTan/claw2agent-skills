...
user-invocable: true
metadata: {"openclaw.category":"data-research","openclaw.risk":"L0","openclaw.layer":"L1","openclaw.tags":"papers, citations, research, academic","openclaw.requires":{"env":["SEMANTIC_SCHOLAR_API_API_KEY"]}}
---
...
- "search papers" related requests
- "get paper" related requests
- "get citations" related requests
- User explicitly mentions "Semantic" or the related platform/service
...
```json
{
  "action": "<one of: search_papers | get_paper | get_citations | get_references>",
...
const result = await execute(
  { "action": "search_papers", "query": "example" },
...
const result = await execute(
  { action: "search_papers" },  // missing required params
  context
);
