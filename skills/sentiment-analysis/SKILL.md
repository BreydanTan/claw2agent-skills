...
description: Analyze text sentiment, emotion, and tone.
user-invocable: true
metadata: {"openclaw.category":"ai-ml","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"sentiment, nlp, ai, analysis","openclaw.requires":{"env":["SENTIMENT_ANALYSIS_API_KEY"]}}
---
...
- "analyze text" related requests
- "analyze batch" related requests
- "detect language" related requests
- User explicitly mentions "Sentiment" or the related platform/service
...
```json
{
  "action": "<one of: analyze_text | analyze_batch | detect_language | extract_entities>",
...
const result = await execute(
  { action: "analyze_text" },  // missing required params
  context
);
