---
name: trello-manager
description: Manage Trello boards, lists, and cards via the Trello API. List boards, get board info, manage lists and cards, move cards between lists, and add comments. Uses injected provider client for API access (BYOK).
user-invocable: true
metadata: {"openclaw.category":"productivity","openclaw.risk":"L0","openclaw.layer":"L1","openclaw.tags":"trello, boards, cards, lists, kanban, project-management, productivity","openclaw.requires":{"config":["providerClient"]}}
---
...
**What it does:**
Manage Trello boards, lists, and cards via the Trello API. List boards, get board info, manage lists and cards, move cards between lists, and add comments. Uses injected provider client for API access (BYOK).
...
- "list boards" related requests
- "get board" related requests
- "list lists" related requests
- User explicitly mentions "Trello" or the related platform/service
...
```json
{
  "action": "<one of: list_boards | get_board | list_lists | list_cards | get_card | create_card | update_card | move_card | add_comment>",
...
const result = await execute(
  { action: "list_boards" },  // missing required params
  context
);
