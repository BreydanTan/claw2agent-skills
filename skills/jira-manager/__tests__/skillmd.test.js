...
const HANDLER_ACTIONS = ["list_projects","get_issue","create_issue","update_issue","transition_issue","search_issues","add_comment","list_transitions","assign_issue"];
...
describe('jira-manager: SKILL.md exists', () => {
  beforeEach(() => {});
...
describe('jira-manager: SKILL.md frontmatter', () => {
  beforeEach(() => {});
  let content, fm;
  ...
  it('frontmatter.name matches slug', () => {
    assert.equal(fm?.name, 'jira-manager');
  });
...
describe('jira-manager: SKILL.md metadata JSON', () => {
  beforeEach(() => {});
  let meta = null;
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
describe('jira-manager: SKILL.md body sections', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
describe('jira-manager: SKILL.md action coverage', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
