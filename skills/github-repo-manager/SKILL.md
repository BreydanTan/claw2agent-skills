...
description: Manage GitHub repositories via the GitHub API. Get repo info, list repos, create and list issues, create and list pull requests, and search code. Uses injected provider client for API access (BYOK).
user-invocable: true
metadata: {"openclaw.category":"development-devops","openclaw.risk":"L2","openclaw.layer":"L1","openclaw.tags":"github, repository, issues, pull-requests, code-search, developer, git, devops","openclaw.requires":{"config":["providerClient"]}}
---
...
**What it does:**
Manage GitHub repositories via the GitHub API. Get repo info, list repos, create and list issues, create and list pull requests, and search code. Uses injected provider client for API access (BYOK).
...
```json
{
  "action": "<one of: get_repo | list_repos | create_issue | list_issues | get_issue | create_pr | list_prs | get_pr | search_code>",
