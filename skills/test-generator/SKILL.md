---
name: test-generator
description: Automatically generate unit and integration tests for code.
user-invocable: true
metadata: {"openclaw.category":"development-devops","openclaw.risk":"L2","openclaw.layer":"L1","openclaw.tags":"testing, automation, code, devops","openclaw.requires":{"env":["TEST_GENERATOR_API_KEY"]}}
---
...
- "generate tests" related requests
- "analyze coverage" related requests
- "suggest cases" related requests
- User explicitly mentions "Test" or the related platform/service
...
```json
{
  "action": "<one of: generate_tests | analyze_coverage | suggest_cases | validate_tests>",
...
const result = await execute(
  { action: "generate_tests" },  // missing required params
  context
);
