---
name: memory-manager
description: Persistent key-value memory storage using a local JSON file. Store, retrieve, search, list, and delete entries with automatic timestamping.
user-invocable: true
metadata: {"openclaw.category":"utilities-knowledge","openclaw.risk":"L0","openclaw.layer":"L0","openclaw.tags":"memory, storage, persistence, key-value, data","openclaw.requires":{}}
---
...
**What it does:**
Persistent key-value memory storage using a local JSON file. Store, retrieve, search, list, and delete entries with automatic timestamping.
...
- User explicitly mentions "Memory" or the related platform/service
...
const result = await execute(
  { action: "store" },  // missing required params
  context
);
