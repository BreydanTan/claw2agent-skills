---
name: code-sandbox-e2b
description: Run code in isolated sandboxes via E2B platform.
user-invocable: true
metadata: {"openclaw.category":"development-devops","openclaw.risk":"L2","openclaw.layer":"L1","openclaw.tags":"sandbox, e2b, code, execution","openclaw.requires":{"env":["CODE_SANDBOX_E2B_API_KEY"]}}
---
...
- "create sandbox" related requests
- "exec code" related requests
...
```json
{
  "action": "<one of: create_sandbox | exec_code | upload_file | close_sandbox>",
...
const result = await execute(
  { action: "create_sandbox" },  // missing required params
  context
);
