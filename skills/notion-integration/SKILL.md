...
description: Interact with Notion workspaces via the Notion API. Search pages and databases, get/create/update pages, query databases, create database entries, and list blocks. Uses injected provider client for API access (BYOK).
user-invocable: true
metadata: {"openclaw.category":"productivity","openclaw.risk":"L0","openclaw.layer":"L1","openclaw.tags":"notion, workspace, pages, databases, blocks, productivity, wiki, knowledge-base","openclaw.requires":{"config":["providerClient"]}}
---
...
**What it does:**
Interact with Notion workspaces via the Notion API. Search pages and databases, get/create/update pages, query databases, create database entries, and list blocks. Uses injected provider client for API access (BYOK).
...
```json
{
  "action": "<one of: search | get_page | create_page | update_page | get_database | query_database | create_database_entry | list_blocks>",
