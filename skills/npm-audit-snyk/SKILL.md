---
name: npm-audit-snyk
description: Scan dependencies for security vulnerabilities via npm audit or Snyk.
user-invocable: true
metadata: {"openclaw.category":"development-devops","openclaw.risk":"L2","openclaw.layer":"L1","openclaw.tags":"npm, snyk, security, audit","openclaw.requires":{"env":["NPM_AUDIT_SNYK_API_KEY"]}}
---
...
- "audit deps" related requests
- "get advisories" related requests
- "check license" related requests
- User explicitly mentions "NPM" or the related platform/service
...
```json
{
  "action": "<one of: audit_deps | get_advisories | check_license | fix_vulnerabilities>",
...
const result = await execute(
  { action: "audit_deps" },  // missing required params
  context
);
