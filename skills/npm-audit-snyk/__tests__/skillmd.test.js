...
const HANDLER_ACTIONS = ["audit_deps","get_advisories","check_license","fix_vulnerabilities"];
...
describe('npm-audit-snyk: SKILL.md exists', () => {
  beforeEach(() => {});
...
describe('npm-audit-snyk: SKILL.md frontmatter', () => {
  beforeEach(() => {});
  let content, fm;
  ...
  it('frontmatter.name matches slug', () => {
    assert.equal(fm?.name, 'npm-audit-snyk');
  });
...
describe('npm-audit-snyk: SKILL.md metadata JSON', () => {
  beforeEach(() => {});
  let meta = null;
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
describe('npm-audit-snyk: SKILL.md body sections', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
describe('npm-audit-snyk: SKILL.md action coverage', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
