...
description: Extract dominant colors and generate palettes from images.
user-invocable: true
metadata: {"openclaw.category":"iot-media","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"color, palette, image, design","openclaw.requires":{"env":["COLOR_PALETTE_EXTRACTOR_API_KEY"]}}
---
...
- "extract palette" related requests
- "get complementary" related requests
- "analyze image" related requests
- User explicitly mentions "Color" or the related platform/service
...
```json
{
  "action": "<one of: extract_palette | get_complementary | analyze_image | generate_palette>",
...
const result = await execute(
  { action: "extract_palette" },  // missing required params
  context
);
