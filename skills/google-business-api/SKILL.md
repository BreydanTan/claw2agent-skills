...
user-invocable: true
metadata: {"openclaw.category":"social","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"google, business, reviews, local","openclaw.requires":{"env":["GOOGLE_BUSINESS_API_API_KEY"]}}
---
...
- "get listing" related requests
- "list reviews" related requests
- "reply review" related requests
...
```json
{
  "action": "<one of: get_listing | list_reviews | reply_review | create_post>",
...
const result = await execute(
  { action: "get_listing" },  // missing required params
  context
);
