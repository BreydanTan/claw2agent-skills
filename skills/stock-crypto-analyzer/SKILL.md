...
description: Real-time stock and cryptocurrency quotes, technical analysis (SMA, RSI, MACD, Bollinger Bands), symbol comparison, and price alert watchlists. Requires a platform gateway adapter for market data access.
user-invocable: true
metadata: {"openclaw.category":"finance","openclaw.risk":"L1","openclaw.layer":"L2","openclaw.tags":"stocks, crypto, finance, market-data, technical-analysis, trading, watchlist, alerts","openclaw.requires":{}}
---
...
**What it does:**
Real-time stock and cryptocurrency quotes, technical analysis (SMA, RSI, MACD, Bollinger Bands), symbol comparison, and price alert watchlists. Requires a platform gateway adapter for market data access.
...
- "quote" related requests
- "analyze" related requests
- "compare" related requests
- User explicitly mentions "Stock" or the related platform/service
...
```json
{
  "action": "<one of: quote | analyze | compare | watchlist_add | watchlist_remove | watchlist_list | alert>",
...
const result = await execute(
  { action: "quote" },  // missing required params
  context
);
