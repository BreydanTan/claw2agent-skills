...
const sample_create_post = {"data":{"publish_id":"pub123"}};
const sample_get_post_status = {"data":{"status":"PUBLISH_COMPLETE","publish_id":"pub123"}};
const sample_list_videos = {"data":{"videos":[{"id":"vid1","title":"Test Video"}],"cursor":20,"has_more":false}};
const sample_get_analytics = {"data":{"videos":[{"id":"vid1","view_count":1000,"like_count":50}]}};

// 1. Action validation
describe('tiktok-content-api: action validation', () => {
  beforeEach(() => {});
...
describe('tiktok-content-api: PROVIDER_NOT_CONFIGURED', () => {
  beforeEach(() => {});
  it('should fail create_post without client', async () => {
    const r = await execute({ action: 'create_post', videoUrl: 'test' }, {});
    assert.equal(r.metadata.success, false);
    assert.equal(r.metadata.error.code, 'PROVIDER_NOT_CONFIGURED');
  });

  it('should fail get_post_status without client', async () => {
    const r = await execute({ action: 'get_post_status', publishId: 'test' }, {});
    assert.equal(r.metadata.success, false);
    assert.equal(r.metadata.error.code, 'PROVIDER_NOT_CONFIGURED');
  });

  it('should fail list_videos without client', async () => {
    const r = await execute({ action: 'list_videos' }, {});
    assert.equal(r.metadata.success, false);
    assert.equal(r.metadata.error.code, 'PROVIDER_NOT_CONFIGURED');
  });

  it('should fail get_analytics without client', async () => {
    const r = await execute({ action: 'get_analytics', videoId: 'test' }, {});
    assert.equal(r.metadata.success, false);
    assert.equal(r.metadata.error.code, 'PROVIDER_NOT_CONFIGURED');
  });
});
...
describe('tiktok-content-api: create_post', () => {
  beforeEach(() => {});

  it('should execute create_post successfully', async () => {
    const ctx = mockContext(sample_create_post);
    const r = await execute({ action: 'create_post', videoUrl: 'test' }, ctx);
  ...
  it('should reject non-string required params for create_post', async () => {
    const ctx = mockContext(sample_create_post);
    const r = await execute({ action: 'create_post', videoUrl: 123 }, ctx);
    assert.equal(r.metadata.success, false);
  });
});

describe('tiktok-content-api: get_post_status', () => {
  beforeEach(() => {});

  it('should execute get_post_status successfully', async () => {
    const ctx = mockContext(sample_get_post_status);
    const r = await execute({ action: 'get_post_status', publishId: 'test' }, ctx);
    assert.equal(r.metadata.success, true);
    assert.equal(r.metadata.action, 'get_post_status');
    assert.ok(r.metadata.timestamp);
  });

  it('should reject missing required params for get_post_status', async () => {
    const ctx = mockContext(sample_get_post_status);
    const r = await execute({ action: 'get_post_status' }, ctx);
    assert.equal(r.metadata.success, false);
    assert.equal(r.metadata.error, 'INVALID_INPUT');
  });

  it('should reject non-string required params for get_post_status', async () => {
    const ctx = mockContext(sample_get_post_status);
    const r = await execute({ action: 'get_post_status', publishId: 123 }, ctx);
    assert.equal(r.metadata.success, false);
  });
});

describe('tiktok-content-api: list_videos', () => {
  beforeEach(() => {});

  it('should execute list_videos successfully', async () => {
    const ctx = mockContext(sample_list_videos);
    const r = await execute({ action: 'list_videos' }, ctx);
    assert.equal(r.metadata.success, true);
    assert.equal(r.metadata.action, 'list_videos');
    assert.ok(r.metadata.timestamp);
  });
});

describe('tiktok-content-api: get_analytics', () => {
  beforeEach(() => {});

  it('should execute get_analytics successfully', async () => {
    const ctx = mockContext(sample_get_analytics);
    const r = await execute({ action: 'get_analytics', videoId: 'test' }, ctx);
    assert.equal(r.metadata.success, true);
  ...
  it('should reject non-string required params for get_analytics', async () => {
    const ctx = mockContext(sample_get_analytics);
    const r = await execute({ action: 'get_analytics', videoId: 123 }, ctx);
    assert.equal(r.metadata.success, false);
  });
});

// N+1. Timeout
describe('tiktok-content-api: timeout', () => {
  beforeEach(() => {});
  it('should timeout on create_post', async () => {
    const r = await execute({ action: 'create_post', videoUrl: 'test' }, mockContextTimeout());
    assert.equal(r.metadata.success, false);
    assert.equal(r.metadata.error, 'TIMEOUT');
  });

  it('should timeout on get_post_status', async () => {
    const r = await execute({ action: 'get_post_status', publishId: 'test' }, mockContextTimeout());
    assert.equal(r.metadata.success, false);
    assert.equal(r.metadata.error, 'TIMEOUT');
  });

  it('should timeout on list_videos', async () => {
    const r = await execute({ action: 'list_videos' }, mockContextTimeout());
    assert.equal(r.metadata.success, false);
    assert.equal(r.metadata.error, 'TIMEOUT');
  });

  it('should timeout on get_analytics', async () => {
    const r = await execute({ action: 'get_analytics', videoId: 'test' }, mockContextTimeout());
    assert.equal(r.metadata.success, false);
    assert.equal(r.metadata.error, 'TIMEOUT');
  });
});
...
// N+2. Network errors
describe('tiktok-content-api: network errors', () => {
  beforeEach(() => {});
  it('should return UPSTREAM_ERROR', async () => {
    const r = await execute({ action: 'create_post', videoUrl: 'test' }, mockContextError(new Error('Connection refused')));
...
// N+3. getClient
describe('tiktok-content-api: getClient', () => {
  beforeEach(() => {});
...
// N+4. redactSensitive
describe('tiktok-content-api: redactSensitive', () => {
  beforeEach(() => {});
...
// N+5. resolveTimeout
describe('tiktok-content-api: resolveTimeout', () => {
  beforeEach(() => {});
...
// N+6. validate()
describe('tiktok-content-api: validate()', () => {
  beforeEach(() => {});
  ...
  it('create_post requires params', () => { assert.equal(validate({ action: 'create_post' }).valid, false); assert.equal(validate({ action: 'create_post', videoUrl: 'test' }).valid, true); });
  it('get_post_status requires params', () => { assert.equal(validate({ action: 'get_post_status' }).valid, false); assert.equal(validate({ action: 'get_post_status', publishId: 'test' }).valid, true); });
  it('list_videos valid with no params', () => { assert.equal(validate({ action: 'list_videos' }).valid, true); });
  it('get_analytics requires params', () => { assert.equal(validate({ action: 'get_analytics' }).valid, false); assert.equal(validate({ action: 'get_analytics', videoId: 'test' }).valid, true); });
});

// N+7. meta export
describe('tiktok-content-api: meta export', () => {
  beforeEach(() => {});
  it('name', () => { assert.equal(meta.name, 'tiktok-content-api'); });
...
describe('tiktok-content-api: gatewayClient fallback', () => {
  beforeEach(() => {});
...
describe('tiktok-content-api: providerNotConfiguredError', () => {
  beforeEach(() => {});
...
// N+10. constants
describe('tiktok-content-api: constants', () => {
  beforeEach(() => {});
  it('VALID_ACTIONS', () => { assert.deepEqual(VALID_ACTIONS, ['create_post', 'get_post_status', 'list_videos', 'get_analytics']); });
});

// N+11. request path verification
describe('tiktok-content-api: request path verification', () => {
  beforeEach(() => {});
  it('should call correct path for create_post', async () => {
    ...
    await execute({ action: 'create_post', videoUrl: 'test' }, ctx);
    assert.ok(calledPath !== null);
  });

  it('should call correct path for get_post_status', async () => {
    let calledPath = null;
    const ctx = { providerClient: { request: async (m, p) => { calledPath = p; return sample_get_post_status; } }, config: { timeoutMs: 5000 } };
    await execute({ action: 'get_post_status', publishId: 'test' }, ctx);
    assert.ok(calledPath !== null);
  });

  it('should call correct path for list_videos', async () => {
    let calledPath = null;
    const ctx = { providerClient: { request: async (m, p) => { calledPath = p; return sample_list_videos; } }, config: { timeoutMs: 5000 } };
    await execute({ action: 'list_videos' }, ctx);
    assert.ok(calledPath !== null);
  });
  ...
  it('should call correct path for get_analytics', async () => {
    ...
    await execute({ action: 'get_analytics', videoId: 'test' }, ctx);
    assert.ok(calledPath !== null);
  });
});
