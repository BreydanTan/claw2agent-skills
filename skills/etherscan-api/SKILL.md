---
name: etherscan-api
description: Access Ethereum blockchain data including balances, transactions, and gas prices
user-invocable: true
metadata: {"openclaw.category":"utilities-knowledge","openclaw.risk":"L0","openclaw.layer":"L1","openclaw.tags":"crypto, ethereum, blockchain, etherscan","openclaw.requires":{"config":["providerClient"]}}
---
...
**What it does:**
Access Ethereum blockchain data including balances, transactions, and gas prices
...
- "get balance" related requests
- "get transactions" related requests
- "get gas price" related requests
- User explicitly mentions "Etherscan" or the related platform/service
...
```json
{
  "action": "<one of: get_balance | get_transactions | get_gas_price | get_token_balance | get_block>",
...
const result = await execute(
  { action: "get_balance" },  // missing required params
  context
);
