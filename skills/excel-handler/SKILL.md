---
name: excel-handler
description: Read, write, and analyze CSV files. Supports parsing CSV to JSON, converting JSON data to CSV format, and computing basic statistics on CSV datasets.
user-invocable: true
metadata: {"openclaw.category":"data-research","openclaw.risk":"L0","openclaw.layer":"L0","openclaw.tags":"csv, excel, spreadsheet, data, parse, statistics","openclaw.requires":{}}
---
...
**What it does:**
Read, write, and analyze CSV files. Supports parsing CSV to JSON, converting JSON data to CSV format, and computing basic statistics on CSV datasets.
...
- "read" related requests
- "write" related requests
...
```json
{
  "action": "<one of: read | write | analyze>",
...
const result = await execute(
  { action: "read" },  // missing required params
  context
);
