---
name: quora-zhihu-manager
description: Manage Q&A content on Quora and Zhihu platforms.
user-invocable: true
metadata: {"openclaw.category":"social","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"quora, zhihu, qa, social","openclaw.requires":{"env":["QUORA_ZHIHU_MANAGER_API_KEY"]}}
---
...
- "search questions" related requests
- "get answers" related requests
- "post answer" related requests
- User explicitly mentions "Quora/Zhihu" or the related platform/service
...
```json
{
  "action": "<one of: search_questions | get_answers | post_answer | get_trending>",
...
const result = await execute(
  { "action": "search_questions", "query": "example" },
...
const result = await execute(
  { action: "search_questions" },  // missing required params
  context
);
