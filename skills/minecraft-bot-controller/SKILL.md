...
description: Control Minecraft bots for building, mining, and exploring.
user-invocable: true
metadata: {"openclaw.category":"iot-media","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"minecraft, bot, gaming, automation","openclaw.requires":{"env":["MINECRAFT_BOT_CONTROLLER_API_KEY"]}}
---
...
- "send command" related requests
- "get status" related requests
- "navigate to" related requests
- User explicitly mentions "Minecraft" or the related platform/service
...
```json
{
  "action": "<one of: send_command | get_status | navigate_to | get_inventory>",
...
const result = await execute(
  { action: "send_command" },  // missing required params
  context
);
