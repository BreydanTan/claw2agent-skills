...
const HANDLER_ACTIONS = ["generate","chat","list_models","pull_model"];
...
describe('ollama-local-llm: SKILL.md exists', () => {
  beforeEach(() => {});
...
describe('ollama-local-llm: SKILL.md frontmatter', () => {
  beforeEach(() => {});
  let content, fm;
  ...
  it('frontmatter.name matches slug', () => {
    assert.equal(fm?.name, 'ollama-local-llm');
  });
...
describe('ollama-local-llm: SKILL.md metadata JSON', () => {
  beforeEach(() => {});
  let meta = null;
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
describe('ollama-local-llm: SKILL.md body sections', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
describe('ollama-local-llm: SKILL.md action coverage', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
