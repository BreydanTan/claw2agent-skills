---
name: excel-api
description: Excel/spreadsheet manipulation skill with JSON data structures representing spreadsheets
user-invocable: true
metadata: {"openclaw.category":"productivity","openclaw.risk":"L0","openclaw.layer":"L0","openclaw.tags":"excel, spreadsheet, csv, workbook, formulas, data","openclaw.requires":{}}
---
...
**What it does:**
Excel/spreadsheet manipulation skill with JSON data structures representing spreadsheets
...
- "create workbook" related requests
- "add sheet" related requests
- "set cells" related requests
- User explicitly mentions "Excel" or the related platform/service
...
```json
{
  "action": "<one of: create_workbook | add_sheet | set_cells | get_sheet | compute_formula | export_csv | list_workbooks>",
...
const result = await execute(
  { action: "create_workbook" },  // missing required params
  context
);
