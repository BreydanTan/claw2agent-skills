---
name: plex-api
description: Browse and manage media libraries on Plex Media Server.
user-invocable: true
metadata: {"openclaw.category":"iot-media","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"plex, media, streaming, library","openclaw.requires":{"env":["PLEX_API_API_KEY"]}}
---
...
- "get libraries" related requests
- "search media" related requests
- "get recently added" related requests
- User explicitly mentions "Plex" or the related platform/service
...
```json
{
  "action": "<one of: get_libraries | search_media | get_recently_added | get_sessions>",
...
const result = await execute(
  { action: "get_libraries" },  // missing required params
  context
);
