---
name: data-analyzer
description: Analyze JSON data arrays with operations like summary, average, count, filter, sort, and groupBy. Parses JSON input and performs statistical and structural analysis on datasets.
user-invocable: true
metadata: {"openclaw.category":"data-research","openclaw.risk":"L0","openclaw.layer":"L0","openclaw.tags":"data, analysis, statistics, json, filter, sort, aggregate","openclaw.requires":{}}
---
...
**What it does:**
Analyze JSON data arrays with operations like summary, average, count, filter, sort, and groupBy. Parses JSON input and performs statistical and structural analysis on datasets.
...
- "summary" related requests
- "average" related requests
- "count" related requests
- User explicitly mentions "Data" or the related platform/service
...
```json
{
  "action": "<one of: summary | average | count | filter | sort>",
...
const result = await execute(
  { action: "summary" },  // missing required params
  context
);
