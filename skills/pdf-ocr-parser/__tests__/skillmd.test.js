...
const HANDLER_ACTIONS = ["parse_pdf","parse_image","extract_tables","get_metadata","list_languages"];
...
describe('pdf-ocr-parser: SKILL.md exists', () => {
  beforeEach(() => {});
...
describe('pdf-ocr-parser: SKILL.md frontmatter', () => {
  beforeEach(() => {});
  let content, fm;
  ...
  it('frontmatter.name matches slug', () => {
    assert.equal(fm?.name, 'pdf-ocr-parser');
  });
...
describe('pdf-ocr-parser: SKILL.md metadata JSON', () => {
  beforeEach(() => {});
  let meta = null;
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
describe('pdf-ocr-parser: SKILL.md body sections', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
describe('pdf-ocr-parser: SKILL.md action coverage', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
