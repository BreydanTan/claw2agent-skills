...
user-invocable: true
metadata: {"openclaw.category":"ai-ml","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"vision, ai, image, analysis","openclaw.requires":{"env":["VISION_MODEL_API_KEY"]}}
---
...
- "detect objects" related requests
- "extract text" related requests
- User explicitly mentions "Vision" or the related platform/service
...
```json
{
  "action": "<one of: analyze_image | detect_objects | extract_text | compare_images>",
...
const result = await execute(
  { action: "analyze_image" },  // missing required params
  context
);
