...
description: YouTube Data API interaction skill. Search videos, retrieve video and channel details, list comments, get trending videos, browse playlists, and list channel videos. Layer 1 skill using provider client for API access.
user-invocable: true
metadata: {"openclaw.category":"social","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"youtube, video, api, search, channels, playlists, comments","openclaw.requires":{"env":["YOUTUBE_DATA_API_API_KEY"]}}
---
...
**What it does:**
YouTube Data API interaction skill. Search videos, retrieve video and channel details, list comments, get trending videos, browse playlists, and list channel videos. Layer 1 skill using provider client for API access.
...
- "search videos" related requests
- "get video" related requests
- "get channel" related requests
- User explicitly mentions "YouTube" or the related platform/service
...
```json
{
  "action": "<one of: search_videos | get_video | get_channel | get_comments | get_trending | get_playlist | get_channel_videos>",
...
const result = await execute(
  { "action": "search_videos", "query": "example" },
...
const result = await execute(
  { action: "search_videos" },  // missing required params
  context
);
