---
name: log-monitor
description: Monitor, search, and analyze application logs.
user-invocable: true
metadata: {"openclaw.category":"development-devops","openclaw.risk":"L2","openclaw.layer":"L1","openclaw.tags":"logs, monitoring, devops, analysis","openclaw.requires":{"env":["LOG_MONITOR_API_KEY"]}}
---
...
- "search logs" related requests
- "get stats" related requests
- "create alert" related requests
- User explicitly mentions "Log" or the related platform/service
...
```json
{
  "action": "<one of: search_logs | get_stats | create_alert | list_alerts>",
...
const result = await execute(
  { "action": "search_logs", "query": "example" },
...
const result = await execute(
  { action: "search_logs" },  // missing required params
  context
);
