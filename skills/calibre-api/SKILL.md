---
name: calibre-api
description: Browse and manage ebook libraries via Calibre Content Server.
user-invocable: true
metadata: {"openclaw.category":"iot-media","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"calibre, ebook, library, reading","openclaw.requires":{"env":["CALIBRE_API_API_KEY"]}}
---
...
- "list books" related requests
- "get book" related requests
- "get categories" related requests
- User explicitly mentions "Calibre" or the related platform/service
...
```json
{
  "action": "<one of: list_books | get_book | get_categories | search_books>",
...
const result = await execute(
  { action: "list_books" },  // missing required params
  context
);
