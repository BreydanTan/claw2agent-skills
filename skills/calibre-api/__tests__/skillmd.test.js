...
const HANDLER_ACTIONS = ["list_books","get_book","get_categories","search_books"];
...
describe('calibre-api: SKILL.md exists', () => {
  beforeEach(() => {});
...
describe('calibre-api: SKILL.md frontmatter', () => {
  beforeEach(() => {});
  let content, fm;
  ...
  it('frontmatter.name matches slug', () => {
    assert.equal(fm?.name, 'calibre-api');
  });
...
describe('calibre-api: SKILL.md metadata JSON', () => {
  beforeEach(() => {});
  let meta = null;
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
describe('calibre-api: SKILL.md body sections', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
describe('calibre-api: SKILL.md action coverage', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
