...
user-invocable: true
metadata: {"openclaw.category":"content","openclaw.risk":"L0","openclaw.layer":"L1","openclaw.tags":"outlook, email, microsoft, graph","openclaw.requires":{"env":["OUTLOOK_MICROSOFT_GRAPH_API_API_KEY"]}}
---
...
- "send email" related requests
- "list messages" related requests
- "get message" related requests
- User explicitly mentions "Outlook" or the related platform/service
...
```json
{
  "action": "<one of: send_email | list_messages | get_message | search_messages>",
...
const result = await execute(
  { action: "send_email" },  // missing required params
  context
);
