...
user-invocable: true
metadata: {"openclaw.category":"data-research","openclaw.risk":"L0","openclaw.layer":"L1","openclaw.tags":"apple, health, fitness, data","openclaw.requires":{"env":["APPLE_HEALTH_PARSER_API_KEY"]}}
---
...
- "parse export" related requests
- "get summary" related requests
- "get workouts" related requests
- User explicitly mentions "Apple" or the related platform/service
...
```json
{
  "action": "<one of: parse_export | get_summary | get_workouts | export_csv>",
...
const result = await execute(
  { action: "parse_export" },  // missing required params
  context
);
