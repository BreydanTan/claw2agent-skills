...
description: Send SMS/MMS messages, retrieve message details, list messages, get account info, and look up phone numbers via Twilio API. Layer 1 skill using provider client for API access.
user-invocable: true
metadata: {"openclaw.category":"communication","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"twilio, sms, mms, messaging, phone, communication","openclaw.requires":{"env":["SMS_SENDER_TWILIO_API_KEY"]}}
---
...
**What it does:**
Send SMS/MMS messages, retrieve message details, list messages, get account info, and look up phone numbers via Twilio API. Layer 1 skill using provider client for API access.
...
- "send sms" related requests
- "get message" related requests
- "list messages" related requests
- User explicitly mentions "SMS" or the related platform/service
...
```json
{
  "action": "<one of: send_sms | get_message | list_messages | send_mms | get_account | check_number>",
...
const result = await execute(
  { action: "send_sms" },  // missing required params
  context
);
