...
**What it does:**
Browser automation via provider client. Navigate pages, take screenshots, extract text and links, fill forms, and evaluate JavaScript snippets. Layer 1 skill using provider client for browser access. HIGH RISK: URL allowlist enforced, no arbitrary code execution.
...
- "navigate" related requests
- "screenshot" related requests
- "get text" related requests
- User explicitly mentions "Playwright" or the related platform/service
...
```json
{
  "action": "<one of: navigate | screenshot | get_text | get_links | fill_form | evaluate>",
...
const result = await execute(
  { action: "navigate" },  // missing required params
  context
);
