...
description: Process and transform video/audio files using FFmpeg commands.
user-invocable: true
metadata: {"openclaw.category":"iot-media","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"ffmpeg, video, audio, media","openclaw.requires":{"env":["VIDEO_EDITOR_FFMPEG_API_KEY"]}}
---
...
- "convert" related requests
- "extract audio" related requests
- User explicitly mentions "Video" or the related platform/service
...
```json
{
  "action": "<one of: get_info | convert | extract_audio | thumbnail>",
...
const result = await execute(
  { action: "get_info" },  // missing required params
  context
);
