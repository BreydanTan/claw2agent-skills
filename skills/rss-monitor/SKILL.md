---
name: rss-monitor
description: Subscribe to, fetch, and monitor RSS/Atom feeds. Parse feed entries, track new items, and filter by keywords.
user-invocable: true
metadata: {"openclaw.category":"data-research","openclaw.risk":"L0","openclaw.layer":"L0","openclaw.tags":"rss, atom, feed, monitor, news, subscription","openclaw.requires":{}}
---
...
**What it does:**
Subscribe to, fetch, and monitor RSS/Atom feeds. Parse feed entries, track new items, and filter by keywords.
...
- "subscribe" related requests
- "unsubscribe" related requests
- "fetch" related requests
- User explicitly mentions "RSS" or the related platform/service
...
```json
{
  "action": "<one of: subscribe | unsubscribe | fetch | list | check>",
...
const result = await execute(
  { action: "subscribe" },  // missing required params
  context
);
