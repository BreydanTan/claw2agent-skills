---
name: sql-analyzer
description: Analyze, optimize, and explain SQL queries.
user-invocable: true
metadata: {"openclaw.category":"development-devops","openclaw.risk":"L2","openclaw.layer":"L1","openclaw.tags":"sql, database, analysis, optimization","openclaw.requires":{"env":["SQL_ANALYZER_API_KEY"]}}
---
...
- "analyze query" related requests
- "explain query" related requests
- "format query" related requests
- User explicitly mentions "SQL" or the related platform/service
...
```json
{
  "action": "<one of: analyze_query | explain_query | format_query | validate_query>",
...
const result = await execute(
  { "action": "analyze_query", "query": "example" },
...
const result = await execute(
  { action: "analyze_query" },  // missing required params
  context
);
