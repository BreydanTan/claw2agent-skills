---
name: pii-redaction
description: Detect and redact personally identifiable information (PII) from text. Supports emails, phone numbers, SSNs, credit cards, IP addresses, and more.
user-invocable: true
metadata: {"openclaw.category":"security","openclaw.risk":"L2","openclaw.layer":"L0","openclaw.tags":"pii, redaction, privacy, security, compliance, gdpr","openclaw.requires":{}}
---
...
**What it does:**
Detect and redact personally identifiable information (PII) from text. Supports emails, phone numbers, SSNs, credit cards, IP addresses, and more.
...
- "redact" related requests
- "report" related requests
- User explicitly mentions "PII" or the related platform/service
...
```json
{
  "action": "<one of: detect | redact | report>",
...
const result = await execute(
  { action: "detect" },  // missing required params
  context
);
