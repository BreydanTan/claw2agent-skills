---
name: frigate-nvr-api
description: Monitor cameras, view events, and manage recordings via Frigate NVR.
user-invocable: true
metadata: {"openclaw.category":"iot-media","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"frigate, nvr, camera, security","openclaw.requires":{"env":["FRIGATE_NVR_API_API_KEY"]}}
---
...
- "get events" related requests
- "get config" related requests
- "get stats" related requests
- User explicitly mentions "Frigate" or the related platform/service
...
```json
{
  "action": "<one of: get_events | get_config | get_stats | get_recordings>",
...
const result = await execute(
  { action: "get_events" },  // missing required params
  context
);
