...
description: Geocode addresses, search places, and calculate routes via Google Maps.
user-invocable: true
metadata: {"openclaw.category":"data-research","openclaw.risk":"L0","openclaw.layer":"L1","openclaw.tags":"google, maps, geocoding, places","openclaw.requires":{"env":["GOOGLE_MAPS_API_API_KEY"]}}
---
...
- "geocode" related requests
- "search places" related requests
- "get directions" related requests
...
```json
{
  "action": "<one of: geocode | search_places | get_directions | get_place_details>",
...
const result = await execute(
  { action: "geocode" },  // missing required params
  context
);
