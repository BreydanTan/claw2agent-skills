---
name: weather-api
description: Weather data retrieval via Open-Meteo API. Supports current weather, forecasts, hourly data, historical data, and location search. Layer 1 skill using provider client for API access.
user-invocable: true
metadata: {"openclaw.category":"iot-media","openclaw.risk":"L1","openclaw.layer":"L1","openclaw.tags":"weather, forecast, climate, temperature, open-meteo","openclaw.requires":{"env":["WEATHER_API_API_KEY"]}}
---
...
**What it does:**
Weather data retrieval via Open-Meteo API. Supports current weather, forecasts, hourly data, historical data, and location search. Layer 1 skill using provider client for API access.
...
- "get current" related requests
- "get forecast" related requests
- "get hourly" related requests
- User explicitly mentions "Weather" or the related platform/service
...
```json
{
  "action": "<one of: get_current | get_forecast | get_hourly | get_historical | search_location | list_variables>",
...
const result = await execute(
  { action: "get_current" },  // missing required params
  context
);
