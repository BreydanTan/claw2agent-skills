...
description: Generate API clients, documentation, and specs from OpenAPI definitions.
user-invocable: true
metadata: {"openclaw.category":"development-devops","openclaw.risk":"L2","openclaw.layer":"L1","openclaw.tags":"swagger, openapi, api, codegen","openclaw.requires":{"env":["SWAGGER_OPENAPI_GENERATOR_API_KEY"]}}
---
...
- "generate client" related requests
- "validate spec" related requests
- "generate docs" related requests
- User explicitly mentions "Swagger/OpenAPI" or the related platform/service
...
```json
{
  "action": "<one of: generate_client | validate_spec | generate_docs | convert_spec>",
...
const result = await execute(
  { action: "generate_client" },  // missing required params
  context
);
