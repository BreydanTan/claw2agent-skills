...
const HANDLER_ACTIONS = ["create_issue","update_issue","list_issues","get_issue","create_project","list_projects","add_comment","search_issues","manage_cycle"];
...
describe('linear-tracker: SKILL.md exists', () => {
  beforeEach(() => {});
...
describe('linear-tracker: SKILL.md frontmatter', () => {
  beforeEach(() => {});
  let content, fm;
  ...
  it('frontmatter.name matches slug', () => {
    assert.equal(fm?.name, 'linear-tracker');
  });
...
describe('linear-tracker: SKILL.md metadata JSON', () => {
  beforeEach(() => {});
  let meta = null;
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
describe('linear-tracker: SKILL.md body sections', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
describe('linear-tracker: SKILL.md action coverage', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
