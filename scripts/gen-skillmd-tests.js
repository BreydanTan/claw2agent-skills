/**
 * gen-skillmd-tests.js
 * Generates __tests__/skillmd.test.js for every skill.
 * Tests: SKILL.md parseable, frontmatter complete, metadata valid,
 *        action coverage, requires field validity.
 *
 * Usage: node --input-type=module < scripts/gen-skillmd-tests.js
 */

import { readdirSync, readFileSync, writeFileSync, mkdirSync, existsSync } from 'node:fs';

function extractActions(handlerPath) {
  try {
...
function genSkillMdTest(slug, actions) {
...
const HANDLER_ACTIONS = ${actionsJson};
...
describe('${slug}: SKILL.md frontmatter', () => {
  beforeEach(() => {});
  let content, fm;
...
describe('${slug}: SKILL.md metadata JSON', () => {
  beforeEach(() => {});
  let meta = null;
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
describe('${slug}: SKILL.md body sections', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
${hasActions ? `describe('${slug}: SKILL.md action coverage', () => {
  beforeEach(() => {});
  let body = '';
  try {
    const content = readFileSync(SKILL_MD_PATH, 'utf8');
...
});` : `describe('${slug}: SKILL.md action coverage', () => {
  beforeEach(() => {});
  it('no handler actions to verify (L0 skill)', () => {
    assert.ok(true);
  });
});`}
`;
...
for (const slug of slugs) {
  try {
    ...
    const actions = extractActions(handlerPath);
    const testDir = `${skillsDir}/${slug}/__tests__`;
    mkdirSync(testDir, { recursive: true });
    writeFileSync(`${testDir}/skillmd.test.js`, genSkillMdTest(slug, actions));
    generated++;
  } catch (e) {
    console.error(`ERROR ${slug}: ${e.message}`);
  }
}
console.log(`Generated ${generated} skillmd.test.js files`);
